...
casper.start(siteRoot, function() {
  this.echo("Crawling "+siteRoot,"GREEN_BAR");
  status = this.status().currentHTTPStatus;
  if(this.status().currentHTTPStatus !== 200){
    this.die(siteRoot +' returned '+this.status().currentHTTPStatus, 1);
  }
});
*/
...
casper.run(function() {
  ...
  this.echo("Finished "+ borked+ " broken urls found.",color);
  this.exit();
});
*/
